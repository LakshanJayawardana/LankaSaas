...
public static class EventStaffingEndpoints
{
    ...
    static async Task<IResult> Assign(Guid eventId,StaffAssignmentRequest r,AppDbContext db,ITenantContext tenant,CancellationToken ct)
    {
        ...
        var conflict=await db.EventStaffAssignments.AnyAsync(x=>x.UserId==r.UserId&&x.Status!=StaffingStatuses.Cancelled&&x.ShiftStartsAt<r.ShiftEndsAt&&r.ShiftStartsAt<x.ShiftEndsAt,ct);if(conflict)return Results.Conflict(new{message="This team member already has an overlapping shift."});
