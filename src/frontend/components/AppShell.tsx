...
export default function AppShell({children}:{children:React.ReactNode}){
 ...
 useEffect(()=>{setEventContextId(pathname==='/events'?'':new URLSearchParams(window.location.search).get('eventId')||'')},[pathname]);
 if(!authChecked)return <main className="auth"><section className="card"><p className="muted">Checking your session...</p></section></main>;
 ...
 return <div className="shell"><AppDialogHost/>
  ...
  <aside className={`side ${menuOpen?'open':''}`}>
   ...
   <nav className="nav" aria-label="Main navigation">{visibleGroups.map(group=>{const expanded=openGroups.includes(group.label);return <section className="nav-group" key={group.label}><button className="nav-group-button" aria-expanded={expanded} onClick={()=>toggle(group.label)}><span>{group.label}</span><span className="nav-chevron" aria-hidden="true">›</span></button>{expanded&&<div className="nav-items">{group.items.map(item=><Link key={item.href} href={item.href} className={pathname.startsWith(item.href)?'active':''} onClick={()=>setMenuOpen(false)}>{item.label}</Link>)}</div>}</section>})}</nav>
  ...
  <main className="main"><header className="top"><button className="icon-btn mobile-only" aria-label="Open menu" onClick={()=>setMenuOpen(true)}>☰</button><span className="mobile-only mobile-top-brand">{branding?.businessName||'WebWaves Digital'}</span><div className="top-spacer"/><Link className="top-profile" href="/profile"><Avatar profile={profile} className="top-avatar"/><span>{name}</span></Link><button className="btn secondary compact" onClick={async()=>{await logout();router.push('/login')}}>Log out</button></header>{access&&(access.isReadOnly||access.status==='PastDue'||access.status==='Cancelled')&&<div className={`access-banner ${access.isReadOnly?'blocked':'warning'}`}><span>{access.message}</span>{admin&&<Link href="/subscription">Manage billing</Link>}</div>}<div className="content">{eventContextId&&<EventContextNav eventId={eventContextId}/>} {denied?<AccessDenied/>:children}</div></main>
 </div>;
}
