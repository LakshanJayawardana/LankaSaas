...
[[ -f "$BACKUP_FILE.sha256" ]] || { echo "Checksum file is missing: $BACKUP_FILE.sha256" >&2;exit 1; }
...
tenant_count="$("${compose[@]}" exec -T db psql -U "$POSTGRES_USER" -d "$verify_db" -Atc 'SELECT count(*) FROM "Tenants";'|tr -d '\r')"
[[ "$tenant_count" =~ ^[0-9]+$ ]] || { echo "Restored tenant count is invalid" >&2;exit 1; }
echo "Restore verification passed in isolated database $verify_db ($table_count public tables, $tenant_count tenants)."
