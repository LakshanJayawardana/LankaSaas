...
export default function Page() {
  ...
  async function cancel() { const confirmed = await appConfirm({ title: 'Cancel recurring billing?', message: 'Your company will keep access until the current billing period ends. Future renewals will stop.', confirmLabel: 'Cancel subscription', cancelLabel: 'Keep subscription', danger: true }); if(!confirmed) return; try { await api('/subscription/cancel', { method: 'POST' }); setError(''); await load() } catch(x) { setError((x as Error).message) } }
