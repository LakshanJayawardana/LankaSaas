...
public record DashboardNextEventDto(Guid Id,string Name,string Venue,DateTimeOffset StartsAt,string Status);
public record DashboardTrendDto(string Month,decimal Revenue,decimal Expenses);
public record DashboardDto(decimal TotalSales,decimal TotalExpenses,int Customers,int Products,DashboardNextEventDto? NextEvent,int UpcomingEvents,int IncompleteChecklistItems,int OverdueInvoices,decimal OutstandingReceivables,List<DashboardTrendDto> Trends);
...
public record AccountingImportLineRequest([Required,MaxLength(30)] string AccountCode,[Required,MaxLength(160)] string AccountName,[Required,MaxLength(20)] string AccountType,[Range(0,double.MaxValue)] decimal Debit,[Range(0,double.MaxValue)] decimal Credit);
public record AccountingImportRequest(DateOnly OpeningDate,[Required,MaxLength(100)] string ImportReference,[MinLength(2)] List<AccountingImportLineRequest> Lines);
public record AccountingImportResultDto(Guid JournalEntryId,int AccountsCreated,int LinesImported,decimal TotalDebit,decimal TotalCredit);
