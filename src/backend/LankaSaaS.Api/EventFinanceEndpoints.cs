...
public static class EventFinanceEndpoints
{
    ...
    static async Task<IResult> ConvertToInvoice(Guid eventId,Guid quotationId,AppDbContext db,ITenantContext tenant,CancellationToken ct)
    {
        ...
        var today=BusinessClock.Today;var invoice=new Invoice{TenantId=tenant.TenantId,EventId=eventId,QuotationId=q.Id,CustomerId=ev.CustomerId,CustomerName=ev.CustomerName,InvoiceNumber=$"{company.InvoicePrefix}-{company.NextInvoiceNumber:00000}",IssueDate=today,DueDate=today.AddDays(company.DefaultPaymentTermsDays),Subtotal=q.Total,Total=q.Total,Notes=q.Notes,Items=q.Items.Select(x=>new InvoiceItem{TenantId=tenant.TenantId,Description=x.Description,Quantity=x.Quantity,UnitPrice=x.UnitPrice,LineSubtotal=x.LineTotal,LineTotal=x.LineTotal}).ToList()};company.NextInvoiceNumber++;q.Status=QuotationStatuses.Converted;db.Invoices.Add(invoice);await AccountingService.Post(db,tenant,invoice.IssueDate,$"Event invoice {invoice.InvoiceNumber}","EventInvoice",invoice.Id,eventId,invoice.InvoiceNumber,[(SystemAccountCodes.Receivables,invoice.Total,0),(SystemAccountCodes.EventRevenue,0,invoice.Total)],ct);await db.SaveChangesAsync(ct);await tx.CommitAsync(ct);return Results.Created($"/api/invoices/{invoice.Id}",new{invoice.Id,invoice.InvoiceNumber});
    }
    ...
    {
        var invoice=await db.Invoices.SingleOrDefaultAsync(x=>x.Id==invoiceId&&x.EventId==eventId,ct);if(invoice is null)return Results.NotFound();if(invoice.Status==InvoiceStatus.Cancelled)return Results.Conflict(new{message="Payments cannot be recorded against a cancelled invoice."});var paid=await db.CustomerPayments.Where(x=>x.InvoiceId==invoiceId).SumAsync(x=>(decimal?)x.Amount,ct)??0;if(paid+r.Amount>invoice.Total)return Results.BadRequest(new{message="Payment exceeds the outstanding invoice balance."});await using var tx=await db.Database.BeginTransactionAsync(ct);var payment=new CustomerPayment{TenantId=tenant.TenantId,EventId=eventId,InvoiceId=invoiceId,Amount=r.Amount,PaymentDate=r.PaymentDate,Method=r.Method.Trim(),Reference=r.Reference?.Trim(),IsDeposit=r.IsDeposit};db.CustomerPayments.Add(payment);invoice.Status=paid+r.Amount==invoice.Total?InvoiceStatus.Paid:InvoiceStatus.Issued;await AccountingService.Post(db,tenant,r.PaymentDate,$"Payment received for {invoice.InvoiceNumber}","CustomerPayment",payment.Id,eventId,r.Reference,[(SystemAccountCodes.Cash,r.Amount,0),(SystemAccountCodes.Receivables,0,r.Amount)],ct);await db.SaveChangesAsync(ct);await tx.CommitAsync(ct);return Results.NoContent();
    }
}
